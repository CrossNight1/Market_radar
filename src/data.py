        ...
        for attempt in range(retries):
            try:
                ...
                if response.status_code == 429:
                    ...
                    logger.warning(f"Rate limited (429). Waiting {retry_after}s... (Attempt {attempt+1}/{retries})")
...
class DataManager:
    ...
    def _sync_data(self, symbol: str, interval: str):
        ...
        try:
            ...
            if not first_ts:
                # No data: fetch 90 days to ensure enough history for long-window indicators
                logger.info(f"Auto-sync: Fetching initial 90 days for {symbol} ({interval})")
                requested_start = end_t - timedelta(days=90)
