...
def nb_rma(x, n):
    ...
    if len(x) < n: return res
    
    first_valid = -1
    for i in range(len(x)):
        if not np.isnan(x[i]):
            if first_valid == -1: first_valid = i
            if i - first_valid + 1 == n:
                s = 0.0
                for j in range(first_valid, i + 1):
                    s += x[j]
                res[i] = s / n
                
                alpha = 1.0 / n
                for j in range(i + 1, len(x)):
                    res[j] = alpha * x[j] + (1.0 - alpha) * res[j-1]
                break
    return res

@nb.njit
def nb_pivots(src, left, right, is_high):
    n = len(src)
...
@nb.njit
def nb_vama(close, vol_vama, targetVol, maMin):
    n = len(close)
    ...
    for i in range(n):
        ...
        if i == 0 or np.isnan(vamaC[i-1]):
            ...
            vamaC[i] = alpha * close[i] + (1.0 - alpha) * vamaC[i-1]
    return vamaC
...
class MetricsEngine:
    ...
    @staticmethod
    def calculate_all_indicators(df: pd.DataFrame, window: int = 20, 
                                 benchmark_returns: Optional[pd.Series] = None,
                                 benchmark_prices: Optional[pd.Series] = None,
                                 interval: str = '1h',
                                 include_metrics: Optional[List[str]] = None) -> pd.DataFrame:
        """
        ...
        # 272. Relative Strength Z-Score
        if should_calc('rel_strength_z') and benchmark_prices is not None:
            # Ratio = Asset Price / BTC Price
            # We must align them by index (open_time)
            # Both are typically already in the same dataframe if processed correctly, 
            # but compute_all_metrics passes them separately.
            
            # For simplicity, we assume they are aligned or we align them here
    ...
    def calculate_breakout_score(df: pd.DataFrame, 
        ...
        if calcMethod == "Atr":
            ...
            stdev_up = pd.Series(close).rolling(len_up).std(ddof=0).values
            stdev_down = pd.Series(close).rolling(len_down).std(ddof=0).values
            slope_up = (stdev_up / len_up) * mult_dyn
            slope_down = (stdev_down / len_down) * mult_dyn
            ...
            var_n_up = s_idx.rolling(len_up).var(ddof=0).values
            var_safe_up = np.where(var_n_up == 0, 1.0, var_n_up)
            slope_up = np.abs(sma_src_n_up - sma_src_up * sma_n_up) / var_safe_up / 2 * mult_dyn
            
            sma_src_n_down = (s_close * s_idx).rolling(len_down).mean().values
            sma_src_down = s_close.rolling(len_down).mean().values
            sma_n_down = s_idx.rolling(len_down).mean().values
            var_n_down = s_idx.rolling(len_down).var(ddof=0).values
    ...
    def compute_all_metrics(self, prices_data: Dict[str, pd.DataFrame], interval: str = '1h', 
        ...
        if benchmark_prices is None and benchmark_symbol in prices_data:
            benchmark_prices = pd.to_numeric(prices_data[benchmark_symbol]['close'], errors='coerce').ffill().fillna(0)
